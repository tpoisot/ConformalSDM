...
conformal.grid[confindex[findall(!isnothing, conf_true)]] .= conf_true[findall(!isnothing, conf_true)]
nodata!(conformal, 1.0)

# Areas that are true
conforange.grid[confindex[findall(!isnothing, conf_true)]] .= 3
# Areas that are false
conforange.grid[confindex[findall(!isnothing, conf_false)]] .= 0
# Areas that are either
conforange.grid[confindex[findall(!isnothing, conf_either)]] .= 1
# Areas that are either but within the range of the BRT
inrange = keys(nodata(mask(conforange .== 1, conforange) & distrib, false))
for ir in inrange
    conforange[ir] = 2
end

fig_variance = Figure(size=(900, 500))
...
legbox = [PolyElement(color=c, strokecolor=:black, strokewidth=1) for c in legcol]
hist!(ax_sab, mask(bsvar, nodata(conforange .== 0, false)), color=legcol[1], strokecolor=:grey, strokewidth=1, bins=22)
hist!(ax_uab, mask(bsvar, nodata(conforange .== 1, false)), color=legcol[2], strokecolor=:grey, strokewidth=1, bins=22)
hist!(ax_upr, mask(bsvar, nodata(conforange .== 2, false)), color=legcol[3], strokecolor=:grey, strokewidth=1, bins=22)
hist!(ax_spr, mask(bsvar, nodata(conforange .== 3, false)), color=legcol[4], strokecolor=:grey, strokewidth=1, bins=22)
...
fig_conformal = Figure(size=(900, 500))
...
hm = heatmap!(ax_confpred, nodata(conformal, 0.0), colormap=probacolor, colorrange=(0.0, 1.0))
...
novelty = similar(temperature, Bool)
...
in_current = Float64[sum(mask(surfacearea, nodata(conforange .== i, false))) for i in [0,1,2,3]]
in_future = Float64[sum(mask(surfacearea, nodata(fconforange .== i, false))) for i in [0,1,2]]
in_novl = Float64[sum(mask(surfacearea, nodata((fconforange.==i)&(novelty), false))) for i in [0,1,2]]
in_seen = Float64[sum(mask(surfacearea, nodata((fconforange.==i)&(!novelty), false))) for i in [0,1,2]]
...
for α in LinRange(0.0, 0.25, 15)
    ...
    partial_conf_pred = MLJ.predict(partial_conf_mach, Xp)
    ...
    partial_inrange = keys(nodata(mask(partial_conforange .== 1, partial_conforange) & distrib, false))
    for ir in partial_inrange
        partial_conforange[ir] = 2
    end
    # Sure/unsure pixels update
    spr = sum(mask(surfacearea, nodata(partial_conforange.==3, false)))# == 0 ? 0.0 : sum(mask(surfacearea, nodata(partial_conforange .== 3, false)))
    upr = sum(mask(surfacearea, nodata(partial_conforange.==2, false)))# == 0 ? 0.0 : sum(mask(surfacearea, nodata(partial_conforange .== 2, false)))
    uab = sum(mask(surfacearea, nodata(partial_conforange.==1, false)))# == 0 ? 0.0 : sum(mask(surfacearea, nodata(partial_conforange .== 1, false)))
    sab = sum(mask(surfacearea, nodata(partial_conforange.==0, false)))# == 0 ? 0.0 : sum(mask(surfacearea, nodata(partial_conforange .== 0, false)))
...
fig_risk = Figure(size=(900, 500))
legcol = [ColorSchemes.get(rangecolor, x, extrema(conforange)) for x in sort(unique(values(conforange)))]
leglab = ["absence", "uncertain (out)", "uncertain (in)", "presence"]
ax_area = Axis(fig_risk[1:2, 1], ylabel="Area (km²)", xlabel="Risk level (α)")
...
hlines!(ax_area, [1e-3 * sum(mask(cellsize(distrib), nodata(distrib, false)))], color=:black, linewidth=2, linestyle=:dash, label="BRT range")
...
Wspr = [sum(abs.(mask(ψ[i], nodata(sure_presence_mask, false)))) for i in eachindex(VARS)]
Wupr = [sum(abs.(mask(ψ[i], nodata(unsure_presence_mask, false)))) for i in eachindex(VARS)]
Wuab = [sum(abs.(mask(ψ[i], nodata(unsure_absence_mask, false)))) for i in eachindex(VARS)]
Wsab = [sum(abs.(mask(ψ[i], nodata(sure_absence_mask, false)))) for i in eachindex(VARS)]
...
fig_global = Figure(size=(900, 500))
legcol = [ColorSchemes.get(rangecolor, x, extrema(conforange)) for x in sort(unique(values(conforange)))]
leglab = ["absence", "uncertain (out)", "uncertain (in)", "presence"]
...
expvar = similar(pred)
expvar.grid[findall(expvar.indices)] .= vx

expl = similar(pred)
expl.grid[findall(expl.indices)] .= vy

frange = maximum(abs.(extrema(vy))) .* (-1, 1)

fig_shapley = Figure(size=(900, 500))
...
density!(ehs, mask(expl, nodata(sure_absence_mask, false)), color=(legcol[1], 0.8), direction=:y, strokecolor=:grey, strokewidth=1, strokearound=true)
density!(ehs, mask(expl, nodata(unsure_absence_mask, false)), color=(legcol[2], 0.8), direction=:y, strokecolor=:grey, strokewidth=1, strokearound=true)
density!(ehs, mask(expl, nodata(unsure_presence_mask, false)), color=(legcol[3], 0.8), direction=:y, strokecolor=:grey, strokewidth=1, strokearound=true)
density!(ehs, mask(expl, nodata(sure_presence_mask, false)), color=(legcol[4], 0.8), direction=:y, strokecolor=:grey, strokewidth=1, strokearound=true)

density!(phs, mask(expl, nodata(sure_absence_mask, false)), color=(legcol[1], 0.8), direction=:x, strokecolor=:grey, strokewidth=1, strokearound=true)
density!(phs, mask(expl, nodata(unsure_absence_mask, false)), color=(legcol[2], 0.8), direction=:x, strokecolor=:grey, strokewidth=1, strokearound=true)
density!(phs, mask(expl, nodata(unsure_presence_mask, false)), color=(legcol[3], 0.8), direction=:x, strokecolor=:grey, strokewidth=1, strokearound=true)
density!(phs, mask(expl, nodata(sure_presence_mask, false)), color=(legcol[4], 0.8), direction=:x, strokecolor=:grey, strokewidth=1, strokearound=true)

scatter!(spl, mask(expvar, nodata(sure_absence_mask, false)), mask(expl, nodata(sure_absence_mask, false)), color=legcol[1], markersize=3, transparency=0.5)
scatter!(spl, mask(expvar, nodata(unsure_absence_mask, false)), mask(expl, nodata(unsure_absence_mask, false)), color=legcol[2], markersize=3, transparency=0.5)
scatter!(spl, mask(expvar, nodata(unsure_presence_mask, false)), mask(expl, nodata(unsure_presence_mask, false)), color=legcol[3], markersize=3, transparency=0.5)
scatter!(spl, mask(expvar, nodata(sure_presence_mask, false)), mask(expl, nodata(sure_presence_mask, false)), color=legcol[4], markersize=3, transparency=0.5)
